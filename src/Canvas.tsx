...
const generateSVGPath = (
  ...
  // console.log('generateSVGPath - path', path)
  const res = createSVGPath(
    path,
    simplifyOptions.simplifyPaths ? simplifyOptions.amount! : 0,
    simplifyOptions.roundPoints!
  );
  // console.log('generateSVGPath - createSVGPath res', res)
...
const generateSVGPaths = (
  ...
  // console.log('genereateSVGPaths - paths', paths)

  return paths.map((i) => ({
    ...i,
    path: i.path
      ? i.path
      : (Array.isArray(i.data) ? i.data : []).reduce(
          (acc: string[], data) => [
            ...acc,
...
// Define a function to handle the eraser logic
const updatePathsWithEraser = (x, y, eraserSize, setPaths) => {
  try {
    // console.log('start of updatePathsWithEraser');
    
    setPaths((prevPaths) => {
      // console.log('start of setPaths for eraser');
      // console.log('prevPaths', prevPaths);

      return prevPaths.reduce((acc: PathType[], p) => {
    ...
    // console.log('setPaths for eraser completed successfully');
  } catch (error) {
    console.log('updatePathsWithEraser - error catch', error);
  }
};

// Define a function to handle the paths update
const updatePaths = (path, simplifyOptions, setPaths, setPath, combineWithLatestPath, color, thickness, opacity) => {
  try {
    // console.log('start of updatePaths');
    
    setPaths((prev) => {
      // console.log('start of setPaths');
      // console.log('prev', prev);
  
      const newSVGPath = generateSVGPath(path, simplifyOptions);
      // console.log('onEnd - newSVGPath', newSVGPath);
  
      if (prev.length === 0) {
        // console.log('prev is empty, creating new path');
        return [
          {
            color,
            path: [newSVGPath],
      ...
      if (
        ...
        // Ensure that lastPath.path and lastPath.data are arrays
        const lastPathPath = Array.isArray(lastPath.path) ? lastPath.path : [];
        const lastPathData = Array.isArray(lastPath.data) ? lastPath.data : [];
  
        // console.log('lastPath.path', lastPath.path);
        // console.log('lastPath.data', lastPath.data);
  
        lastPath.path = [...lastPathPath, newSVGPath];
        lastPath.data = [...lastPathData, path];
    ...
    // console.log('setPaths completed successfully');
    setPath([]);
  } catch (error) {
    console.log('updatePaths - error catch', error);
  }
};
...
const Canvas = forwardRef<CanvasRef, CanvasProps>(
  (
    ...
    const addPath = (newPath: PathType) => {
      // console.log('running addPath - newPath', newPath);
    ...
    const panGesture = Gesture.Pan()
      .onChange(({ x, y }) => {
        switch (tool) {
          case DrawingTool.Brush:
            // console.log('panGesture - x, y', {x, y})

            runOnJS(addPointToPath)(x, y);
            break;
            case DrawingTool.Eraser:
              runOnJS(updatePathsWithEraser)(x, y, eraserSize, setPaths);
              break;
        }
      })
      .onBegin(({ x, y }) => {
        if (tool === DrawingTool.Brush) {
          // console.log('onBegin - x, y', {x, y})
          runOnJS(addPointToPath)(x, y);
        }
      })
      .onEnd(() => {
        if (tool === DrawingTool.Brush) {
          // console.log('onEnd - path', path);
          try {
            // console.log('onEnd - prior to runOnJS');
            runOnJS(updatePaths)(
              path,
              simplifyOptions,
              setPaths,
            ...
            console.log('onEnd - error catch', error);
          }
        }
      })
      ...
      //         try {
      //           setPaths((prev) => {
      //             console.log('start of runOnJS setPaths');
      //             console.log('prev', prev);
      ...
      //           });
      //         } catch (error) {
      //           console.log('runOnJS setPaths - error catch', error);
      //         }
      //       })();
      //     } catch (error) {
      //       console.log('onEnd - error catch', error);
